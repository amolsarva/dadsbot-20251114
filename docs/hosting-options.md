# Hosting options for Vercel-centric builds

The app is a Next.js 14 project with App Router server actions, serverless API routes, and Supabase-backed blob storage. These platforms cover the required deployment + storage combo with minimal code churn.

| Platform | What you get | Gaps / what to configure |
| --- | --- | --- |
| **Vercel** | Git-driven deploys, preview URLs, custom domains, built-in support for Next.js App Router, and seamless environment management. Supabase credentials live in Vercel env vars and diagnostics surface the deployment metadata automatically. | Provide the Supabase URL, service role key, bucket, and `STORAGE_MODE=supabase` via `tmpkeys.txt`; see `docs/vercel-migration-guide.md`. |
