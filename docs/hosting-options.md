...
| --- | --- | --- |
| **Netlify** | Git-driven deploys, preview URLs, custom domains, serverless functions that map to `app/api/*`, and painless integration with Supabase via `tmpkeys.txt`. Diagnostics stay green once the Supabase secrets are in place. | Provide the Supabase URL, service role key, bucket, and `STORAGE_MODE=supabase` in `tmpkeys.txt`; see `docs/netlify-migration-guide.md`. |
