...
import { appendTurn, createSession, finalizeSession, deleteSession } from '@/lib/data'
...
function logDiagnostic(step: string, payload: Record<string, unknown> = {}) {
  console.log(`[diagnostic] ${diagnosticTimestamp()} diagnostics:e2e:${step}`, {
    ...payload,
...
function logDiagnosticError(step: string, error: unknown, payload: Record<string, unknown> = {}) {
  ...
  console.error(`[diagnostic] ${diagnosticTimestamp()} diagnostics:e2e:${step}:error`, {
    ...payload,
    envSummary: diagnosticEnvSummary(),
